...
# -------------------------------------------------------------------
# Разбор аргументов командной строки (Исправленный)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Сертификаты и Настройки
# -------------------------------------------------------------------
install_acme() {
    echo -e "${green}Installing acme.sh for SSL certificate management...${plain}"
    cd ~ || return 1
    curl -s https://get.acme.sh | sh > /dev/null 2>&1
    echo -e "${green}acme.sh обработан.${plain}"
    return 0
}
...
setup_ip_certificate() {
    ...
    ~/.acme.sh/acme.sh --issue -d "${ipv4}" --standalone --server letsencrypt --certificate-profile shortlived --days 6 --httpport 80 --force || true
    ~/.acme.sh/acme.sh --installcert -d "${ipv4}" --key-file "${certDir}/privkey.pem" --fullchain-file "${certDir}/fullchain.pem" 2>&1 || true
    chmod 600 ${certDir}/privkey.pem 2> /dev/null
    chmod 644 ${certDir}/fullchain.pem 2> /dev/null
    ${xui_folder}/x-ui cert -webCert "${certDir}/fullchain.pem" -webCertKey "${certDir}/privkey.pem" || true
}
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}1.${plain} Let's Encrypt for IP Address (6-day validity)"
    echo -e "${green}2.${plain} Skip SSL (Use HTTP)"
    ...
    case "$ssl_choice" in
        1) setup_ip_certificate "${server_ip}"; SSL_HOST="${server_ip}" ;;
        2) SSL_SCHEME="http"; SSL_HOST="${server_ip}" ;;
    esac
}
...
# -------------------------------------------------------------------
# Компоненты: Xray-Core
# -------------------------------------------------------------------
install_xray() {
    local xray_dir="${xui_folder}/bin"
    if [[ -x "${xray_dir}/xray" ]] && "${xray_dir}/xray" -version &>/dev/null; then
        echo -e "${green}Xray-core уже установлен и работает. Пропускаем.${plain}"
        return 0
    fi
...
install_x-ui() {
    ...
    # СТРОГИЙ ПОРЯДОК: Сначала собираем ВСЕ необходимые бинарники на диске
    install_xray
    ...
    # И ТОЛЬКО ТЕПЕРЬ запускаем конфигурирование и первый старт панели!
    config_after_install

    # 🚀 СОЗДАНИЕ И РЕГИСТРАЦИЯ СЛУЖБЫ АВТОЗАПУСКА (SYSTEMD)
    echo -e "${green}Создаем службу автозапуска в systemd...${plain}"
...
EOF
    ...
    echo -e "${green}🎉 Установка полностью завершена! Панель запущена службой.${plain}"
}
...
# -------------------------------------------------------------------
# Точка входа в скрипт (Исправленная)
# -------------------------------------------------------------------
