...
- **Встроенный TUIC v5** — высокопроизводительный прокси на базе QUIC с нативным учётом трафика через UDP-релей, 0-RTT рукопожатиями, контролем перегрузок BBR и маршрутизацией через Xray.
