...
- **Полная обфускация AmneziaWG 2.0** — Jc/Jmin/Jmax (мусорные пакеты), S1–S4 (паддинг пакетов), H1–H4 (магические заголовки) и все 5 слотов сигнатурных пакетов (I1–I5) — каждый можно сгенерировать одним кликом как имитацию DNS/STUN/SIP/QUIC-пакета, настоящий TLS ClientHello браузера Chrome/Firefox/Safari или как чистую случайность — всё редактируется по каждому инбаунду, плюс совместимый с 1.x режим для старых клиентов.
- **Опционально AmneziaWG 3.0 и 3.1.** Селектор версии на инбаунде открывает поля 3.0 — `HeaderProtectionKey`/`ContentPaddingAddition` (плюс переопределение таймеров handshake/rekey/keepalive) — и поля 3.1 — `RandomTrailers`/`DisableCookies`. Каждое включение осознанное и явное, ничего не меняет поведение на проводе само по себе.
...
- **Отдельная IPv6-идентичность и проброс портов по клиентам** тоже работают на встроенном движке — у клиента с IPv6-адресом в `AllowedIPs` появляется собственный алиас исходящего адреса вместо общего хостового, а `ForwardedPorts` открывает настоящий слушатель, который дозванивается обратно в туннель — то же поведение для админа, что и в исходной реализации на модуле ядра, только на новой архитектуре.
...
- **Встроенная «парадная дверь» (реверс-прокси + сайт-заглушка)** — новая вкладка **Настройки → Парадная дверь** поднимает локальный TLS-слушатель, который маршрутизирует по пути: базовый путь панели — в панель, путь подписки — на сервер подписок, все остальные пути — на сайт-заглушку. Укажите его в fallback-поле `target` REALITY-инбаунда, и тот, кто просканирует ваш `:443`, увидит обычный сайт, а не намёк на то, что здесь живёт панель. Заглушкой может быть одна из встроенных страниц, ваш собственный статический сайт, загруженный zip-архивом, или прокси на уже существующий сайт. Сертификат — либо файлы самой панели, либо автоматический выпуск через Let's Encrypt (HTTP-01, так как `:443` занят Xray). Это заменяет написанный вручную конфиг Nginx, который раньше требовался для такой схемы; если ошибка в настройках закроет вам доступ, `x-ui setting -disableFrontProxy` выключает дверь по SSH.
