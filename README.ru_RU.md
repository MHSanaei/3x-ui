...
- **Встроенный реверс-прокси с сайтом-заглушкой** — новая вкладка **Настройки → Реверс-прокси** поднимает локальный TLS-слушатель, который маршрутизирует по пути: базовый путь панели — в панель, путь подписки — на сервер подписок, все остальные пути — на сайт-заглушку. Укажите его в fallback-поле `target` REALITY-инбаунда, и тот, кто просканирует ваш `:443`, увидит обычный сайт, а не намёк на то, что здесь живёт панель. Заглушкой может быть одна из встроенных страниц, ваш собственный статический сайт, загруженный zip-архивом, или прокси на уже существующий сайт. Сертификат — либо файлы самой панели, либо автоматический выпуск через Let's Encrypt (HTTP-01, так как `:443` занят Xray). Это заменяет написанный вручную конфиг Nginx, который раньше требовался для такой схемы; если ошибка в настройках закроет вам доступ, `x-ui setting -disableFrontProxy` выключает его по SSH.
