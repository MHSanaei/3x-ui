...
### Вход через SSO / OIDC (опционально)

Задайте `XUI_OAUTH_ISSUER` **и** `XUI_OAUTH_CLIENT_ID`, чтобы включить единый вход через любой OpenID Connect провайдер (Keycloak, Authentik, Authelia, Google, Azure AD, Okta и т.д.). Claim групп в ID-токене распределяет пользователя по уровню доступа: участники группы администраторов получают полный доступ к панели, а участники пользовательской группы автоматически получают клиента и личный кабинет, где видят только своё подключение (ссылки, QR, трафик). Фоновая задача (примерно раз в 5 минут) также привязывает уже существующих пользователей к инбаундам, добавленным после их входа, — новый подходящий инбаунд доезжает до них без повторного входа.

| Переменная | Описание | По умолчанию |
| --- | --- | --- |
| `XUI_OAUTH_ISSUER` | URL OIDC-провайдера (discovery). Вместе с client id включает SSO | — |
| `XUI_OAUTH_CLIENT_ID` | Идентификатор OAuth2-клиента | — |
| `XUI_OAUTH_CLIENT_SECRET` | Секрет OAuth2-клиента | — |
| `XUI_OAUTH_REDIRECT_URL` | URL callback, зарегистрированный у провайдера | выводится из запроса |
| `XUI_OAUTH_ADMIN_GROUP` | Группа, участники которой получают полный доступ к панели | — |
| `XUI_OAUTH_USER_GROUP` | Группа(ы), участники которых получают личный кабинет (через запятую) | — |
| `XUI_OAUTH_USER_INBOUND_REMARK` | Примечание(я) инбаундов, в которые автосоздаётся клиент (через запятую) | — |
| `XUI_OAUTH_USER_TOTAL_GB` | Лимит трафика для автосозданных клиентов, ГБ (`0` = без лимита) | `0` |
| `XUI_OAUTH_USER_EXPIRY_DAYS` | Срок действия автосозданных клиентов, дней (`0` = бессрочно) | `0` |
| `XUI_OAUTH_USER_LIMIT_IP` | Лимит устройств/IP для автосозданных клиентов (`0` = без лимита) | `0` |
| `XUI_OAUTH_SCOPES` | Scope OAuth2 (через запятую) | `openid,profile,email,groups` |
| `XUI_OAUTH_GROUPS_CLAIM` | Claim ID-токена с группами пользователя | `groups` |
| `XUI_OAUTH_USERNAME_CLAIM` | Claim ID-токена, используемый как имя пользователя | `email` |
