# dune via cloud-init (generic, no golden image)

This is the **secondary** deployment path: a single [`cloud-init.yaml`](cloud-init.yaml)
user-data file that installs dune non-interactively on a fresh Ubuntu/Debian
...
2. cloud-init writes and runs `/opt/dune-bootstrap.sh`, which exports
   `DUNE_NONINTERACTIVE=1` and pipes the project's `install.sh` into `bash`.
3. `install.sh` runs end-to-end with **zero prompts**, picking secure random
   values for any credential you didn't pin.
4. The generated credentials are written to `/etc/dune/install-result.env`
   (mode 600), echoed to the **serial console**, and appended to `/etc/motd`.

Retrieve them after boot with either:

```bash
sudo cat /etc/dune/install-result.env     # over SSH
```
...
Edit the `export DUNE_*` lines inside the `write_files` block of
[`cloud-init.yaml`](cloud-init.yaml). All knobs are optional; unset ⇒ random/secure default.

| Env var | Default | Meaning |
| --- | --- | --- |
| `DUNE_SSL_MODE` | `none` | `none` (plain HTTP), `ip` (Let's Encrypt IP cert), `domain` |
| `DUNE_USERNAME` | random | Admin username |
| `DUNE_PASSWORD` | random | Admin password |
| `DUNE_PANEL_PORT` | random high port | Panel listen port |
| `DUNE_WEB_BASE_PATH` | random | Panel base path (obscures the URL) |
| `DUNE_DOMAIN` | — | Required when `DUNE_SSL_MODE=domain` |
| `DUNE_ACME_EMAIL` | — | Let's Encrypt account email (domain mode) |
| `DUNE_DB_TYPE` / `DUNE_DB_DSN` | `sqlite` | Set `postgres` + DSN to use PostgreSQL |
...
- **Google Cloud (GCE)** — `gcloud compute instances create dune \
