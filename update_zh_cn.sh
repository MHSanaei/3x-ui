...
setup_ip_certificate() {
    ...
    echo -e "${yellow}默认监听端口为 80。如果您选择其他端口，请确保外部端口 80 转发到该端口。${plain}"
...
prompt_and_setup_ssl() {
    ...
    # 如果输入为空或无效（不是 1 或 3），则默认为 2（IP 证书）
...
config_after_update() {
    ...
    if [[ -z "$existing_cert" ]]; then
        ...
        echo -e "${green}     面板访问信息              ${plain}"
        echo -e "${green}═══════════════════════════════════════════${plain}"
        echo -e "${green}访问 URL：https://${cert_domain}:${existing_port}/${existing_webBasePath}${plain}"
        echo -e "${green}═══════════════════════════════════════════${plain}"
    fi
}
...
update_x-ui() {
    ...
    if [[ $? -ne 0 ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            _fail "错误：无法下载 x-ui，请确保您的服务器可以访问 GitHub"
        fi
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            ...
            if [[ $? -ne 0 ]]; then
                _fail "错误：无法下载启动单元 x-ui.rc，请确保您的服务器可以访问 GitHub"
            fi
        fi
