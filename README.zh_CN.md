...
- **完整的 AmneziaWG 2.0 混淆功能**——Jc/Jmin/Jmax（垃圾数据包）、S1–S4（数据包填充）、H1–H4（魔术头部）以及全部 5 个签名数据包插槽（I1–I5)——每个都可一键生成为模拟 DNS/STUN/SIP/QUIC 的数据包、真实的 Chrome/Firefox/Safari TLS ClientHello,或纯随机数据，均可按入站单独编辑，同时为旧版客户端提供 1.x 兼容模式。
