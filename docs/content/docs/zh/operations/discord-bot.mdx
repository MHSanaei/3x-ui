...
1. 在开发者门户左侧导航栏中，进入 **OAuth2** $\rightarrow$ **URL Generator**。
2. 在 **Scopes** 中勾选 `bot`。
...
1. 在 Discord 客户端中开启开发者模式：**用户设置** $\rightarrow$ **高级** $\rightarrow$ **开发者模式**（开启）。
...
1. 在 3x-ui 面板中，打开**面板设置** $\rightarrow$ **Discord 机器人**（或直接访问 `/settings#discord`）。
2. 在**通用**区域：
   - 开启**启用 Discord 通知**。
