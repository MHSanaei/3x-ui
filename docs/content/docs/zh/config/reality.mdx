...
- **旧客户端内核默认被拒。** **最小客户端版本**留空并不是"不限制"：Xray-core 会退回到内置最低值（本文撰写时为 26.3.27）以保证客户端 TLS 指纹的新鲜度，因此 Mihomo、sing-box 等第三方内核即使配置完全正确也会 REALITY 验证失败——表现为客户端超时，只有基于 Xray-core 的应用能连上。只有在必须支持它们时才填 `1.0.0`；这同时也会放行过时的指纹。
