...
| `XUI_SUB_PORT`           | —       | Override the subscription server port (1–65535). Takes precedence over the stored setting. |
...
| `XUI_PANEL_PORT`     | Optional initial web panel port override during unattended install (default: random).         |
| `XUI_SUBSCRIPTION_PORT` | Optional initial subscription server port override during unattended install (default: random). |
