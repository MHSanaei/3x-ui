...
description: 'A separate HTTP/HTTPS server that serves proxy subscription links
  ...
  (randomized on fresh script installations, factory default: 2096) and is
  configured in Settings → Subscription. Fresh panels generate random path
  prefixes for each format; all paths remain configurable. Every subscription
  endpoint sets response headers for client apps to read traffic/expiry info.'
full: true
_openapi:
  preload:
    - ./public/openapi.json
