...
| **Speed Down** | Xray protocols        | Max download rate in Mbps (`0` = unlimited). Requires **Client Speed Limit**. |
| **Speed Up**   | Xray protocols        | Max upload rate in Mbps (`0` = unlimited). Requires **Client Speed Limit**. |
...
- **Speed Down / Speed Up** cap bandwidth for observed client IPs using Linux
  `tc` (HTB on egress, shared ingress police for upload). Enable **Client Speed
  Limit** under Settings → General (default **off**), then restart the panel.
  Requires `tc` and sufficient privileges on the host; it takes over the primary
  interface root qdisc when enabled. Not applied to MTProto-only clients (those
  connections are outside Xray online-stats).
