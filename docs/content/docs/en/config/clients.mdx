...
| **Traffic Ratio** | all                | Multiplier on recorded usage (default `1`). `2` stores/shows twice the real bytes. |
...
- **Traffic / expiry** caps disable the client when hit; a **Reset** period
  auto-renews the quota. **Traffic Ratio** multiplies real deltas before they
  are stored (quota and panel usage both use the billed counters; inbound
  aggregates stay raw).
