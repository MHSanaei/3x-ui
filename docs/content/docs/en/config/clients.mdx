...
- **Traffic / expiry** caps disable the client when hit; a **Reset** period
  auto-renews the quota. Billed usage may be inflated by the attached
  inbound's **Traffic Ratio** (see [Inbounds](/docs/config/inbounds)). When a
  client is on multiple inbounds with different ratios, local accounting uses
  the ratio of the lowest-id inbound for the whole email total.
