...
- **Old client cores rejected by default.** An empty **Min Client Ver** is not
  "no limit": Xray-core falls back to a built-in minimum (26.3.27 at the time of
  writing) that keeps client TLS fingerprints fresh, so third-party cores such as
  Mihomo and sing-box fail REALITY verification even with a correct config —
  clients see timeouts while only Xray-core based apps connect. Set it to `1.0.0`
  only if you must support them; that also re-admits outdated fingerprints.
