...
Only enabled inbounds using **VLESS, VMess, Trojan, Shadowsocks, WireGuard, TUIC, or Hysteria2**
appear in a subscription, ordered by their sub-sort index (TUIC is included in raw links and Clash/Mihomo profiles, but omitted from JSON endpoints). Requesting `subPath`
