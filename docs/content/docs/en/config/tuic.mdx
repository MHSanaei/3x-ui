...
<Callout type="info">
  ...
  - **Traffic accounting & limits**: Total traffic is measured from the sidecar process I/O counters (`/proc/<pid>/io`, **Linux only** — on Windows and macOS the counters read zero, so inbound traffic limits are not enforced). Because upstream `tuic-server` does not provide an internal per-user metrics API, individual client traffic limits (`totalGB`) are not supported for TUIC clients. Client access can be controlled via expiration timestamps (`expiryTime`) and manual enable/disable toggles.
