---
title: TUIC
description: Set up a TUIC inbound in 3x-ui — QUIC congestion control, 0-RTT handshakes, multi-user authentication, and routing client traffic through Xray.
icon: Zap
---
...
<Callout type="info">
  ...
  lifecycle, monitors process health, and reads I/O metrics. Each client's
  authenticated traffic relays through a local loopback SOCKS5 listener
  (`127.0.0.1:63200+id`) into Xray, so client routing, sniffing, and traffic quotas
  are seamlessly integrated with the rest of the panel.
</Callout>
...
<Callout type="info">
  ...
  - **Traffic accounting**: Total traffic is accounted from the sidecar process I/O counters and active connections continuously update client online status.
