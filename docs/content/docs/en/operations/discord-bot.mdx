...
When enabled, the bot listens to commands in the configured Discord channel (supporting both `!` and `/` prefixes):
...
- **Commands do not respond**: Ensure **Message Content Intent** is enabled under the **Bot** tab in Discord Developer Portal, then restart the panel. Discord closes the connection for good when the intent is missing, so the bot does not retry on its own.
