...
// Same event taxonomy as EmailNotifications — only the settingKey for
// threshold events (cpu.high/memory.high) points at the webhook-specific
// threshold setting instead of the SMTP one.
...
export function WebhookNotifications({ allSetting, updateSetting }: Props) {
  const events = allSetting.webhookEnabledEvents || '';
  ...
  function toggle(key: string) {
    const next = selected.includes(key)
        ...
        : [...selected, key];
    updateSetting({ webhookEnabledEvents: next.join(',') });
  }
