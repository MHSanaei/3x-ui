...
export default function InboundFormModal({
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        speedLimit: form.speedLimit || 0,
        speedLimitType: form.speedLimitType || 'all',
        listen: ib.listen,
        port: ib.port,
        protocol: ib.protocol,
        settings,
  ...
  const renderBasicsTab = () => (
    <Form colon={false} labelCol={{ sm: { span: 8 } }} wrapperCol={{ sm: { span: 14 } }}>
      ...
      <Form.Item label={<Tooltip title={t('pages.inbounds.speedLimitDesc')}>{t('pages.inbounds.speedLimit')}</Tooltip>}>
        <Space.Compact block>
          <Select
            value={form.speedLimitType || 'all'}
            style={{ width: 112 }}
            onChange={(v) => { form.speedLimitType = v || 'all'; refresh(); }}
          >
            <Select.Option value="all">{t('pages.inbounds.speedLimitModeAll')}</Select.Option>
            <Select.Option value="up">{t('pages.inbounds.speedLimitModeUp')}</Select.Option>
            <Select.Option value="down">{t('pages.inbounds.speedLimitModeDown')}</Select.Option>
          </Select>
          <InputNumber
