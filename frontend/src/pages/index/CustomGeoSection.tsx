...
import { Alert, Button, Form, Input, message, Modal, Space, Table, Tag, Tooltip, Typography } from 'antd';
...
export default function CustomGeoSection({ active }: CustomGeoSectionProps) {
  ...
  const loadSchedule = useCallback(async () => {
    const msg = await HttpUtil.get('/panel/api/custom-geo/schedule');
    if (msg?.success && typeof msg.obj === 'string') setCronExpr(msg.obj);
  }, []);
  ...
  }, [active, loadList, loadSchedule]);

  async function saveCron() {
    setSavingCron(true);
    try {
      const msg = await HttpUtil.post('/panel/api/custom-geo/schedule', { cron: cronExpr.trim() });
      if (msg?.success) messageApi.success(msg.msg || t('pages.index.customGeoCronSaved'));
      else messageApi.error(msg?.msg || t('pages.index.customGeoCronInvalid'));
    } finally {
      setSavingCron(false);
    }
  }
  ...
  return (
    <div className="custom-geo-section">
      ...
      <div className="custom-geo-cron-block">
        <Form layout="vertical" style={{ margin: 0 }}>
          <Form.Item
            label={
              <span>
                {t('pages.index.customGeoCron')}
                <Typography.Text type="secondary" style={{ marginLeft: 8, fontSize: 12 }}>
                  {t('pages.index.customGeoCronHint')}
                </Typography.Text>
              </span>
            }
