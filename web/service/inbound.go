...
func (s *InboundService) getAllEmails() ([]string, error) {
	...
	var emails []string
	err := db.Raw(`
		SELECT JSON_UNQUOTE(JSON_EXTRACT(client, '$.email')) AS email
		FROM inbounds,
		JSON_TABLE(inbounds.settings, "$.clients[*]"
			COLUMNS (client JSON PATH "$")
		) AS jt
	`).Scan(&emails).Error
	if err != nil {
		return nil, err
...
func (s *InboundService) MigrationRemoveOrphanedTraffics() {
	...
	db.Exec(`
		...
		WHERE email NOT IN (
			SELECT JSON_UNQUOTE(JSON_EXTRACT(client, '$.email'))
			FROM inbounds,
			JSON_TABLE(inbounds.settings, "$.clients[*]"
				COLUMNS (client JSON PATH "$")
			) AS jt
...
func (s *InboundService) GetClientTrafficByID(id string) ([]xray.ClientTraffic, error) {
	...
	err := db.Model(xray.ClientTraffic{}).Where(`
		email IN (
			SELECT JSON_UNQUOTE(JSON_EXTRACT(client, '$.email')) AS email
			FROM inbounds,
			JSON_TABLE(inbounds.settings, "$.clients[*]"
			...
			WHERE JSON_UNQUOTE(JSON_EXTRACT(client, '$.id')) = ?
		)
