...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "client_commands":
		...
		case "scan":
    // Get all website domains from your data source
    webAddrs := []string{"example.com", "google.com", "yahoo.com"}

    // Loop through each domain and check its status
    results := make(map[string]string)
    for _, domain := range webAddrs {
        // Check the domain's status using the getWebsiteStatus() function
        status, err := getWebsiteStatus(domain)
        if err != nil {
    ...
    for domain, result := range results {
        responseMsg += domain + " - " + result + "\n"
    }
    sendMessageToUser(responseMsg)

    // Show a keyboard to allow the user to choose between manual or automated scanning
    numericKeyboardGetIPs := tu.InlineKeyboard(
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("دستی").WithCallbackData("manual"),
            tu.InlineKeyboardButton("اتوماتیک").WithCallbackData("automatic"),
        ),
    )
    message := t.I18nBot("what_operation_do_you_want_to_perform")
    tu.SendMessage(tgbotAdminChatId, message, tu.ModeHTML, true, 0, numericKeyboardGetIPs)
	case "commands":    
