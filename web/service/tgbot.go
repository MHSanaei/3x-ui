...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	keyboard := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tu.InlineKeyboardButton("Mci").WithCallbackData("mci"),
			tu.InlineKeyboardButton("Mtn").WithCallbackData("mtn"),
		),
	)

	// Send the message with the created keyboard
	msg := tgbotapi.NewMessage(chatId, "لطفا یکی از دکمه ها را انتخاب کنید:")
	msg.ReplyMarkup = keyboard
	t.telegramBot.Send(msg)
case "mci":
	ips, err := net.LookupIP("Mci.ircf.space")
	if err != nil {
		logger.Error("Failed to resolve IP address for domain Mci.ircf.space: ", err)
		t.SendAnswer(callbackQuery.Message.Chat.ID, "❗ خطایی در دریافت آدرس IP برای دامنه Mci.ircf.space وجود دارد.", false)
		return
	}
	var output strings.Builder
	output.WriteString("آدرس های IP برای دامنه Mci.ircf.space:\n")
	for _, ip := range ips {
		output.WriteString(fmt.Sprintf("- %s\n", ip.String()))
	}
	t.SendAnswer(callbackQuery.Message.Chat.ID, output.String(), false)
case "mtn":
	ips, err := net.LookupIP("Mtn.ircf.space")
	if err != nil {
		logger.Error("Failed to resolve IP address for domain Mtn.ircf.space: ", err)
		t.SendAnswer(callbackQuery.Message.Chat.ID, "❗ خطایی در دریافت آدرس IP برای دامنه Mtn.ircf.space وجود دارد.", false)
		return
	}
	var output strings.Builder
	output.WriteString("آدرس های IP برای دامنه Mtn.ircf.space:\n")
	for _, ip := range ips {
		output.WriteString(fmt.Sprintf("- %s\n", ip.String()))
	}
	t.SendAnswer(callbackQuery.Message.Chat.ID, output.String(), false)
...
tu.InlineKeyboardButton("🌐 آی پی سالم").WithCallbackData("get_ip"),
