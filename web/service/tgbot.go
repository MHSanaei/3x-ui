...
func (t *Tgbot) answerCommand(message *telego.Message, chatId int64, isAdmin bool) {
	...
	// Check if the message contains the keyword "کلودفلر"
	if strings.Contains(message.Text, "کلودفلر") {
		// create a list of domains to lookup IPs for
		domains := []string{"mci.filna.cloud", "mcih.hamedgeek.top"}

		// loop over all domains and get their IPs
		var msgBuilder strings.Builder
		for _, domain := range domains {
			ips, err := net.LookupIP(domain)
			if err != nil {
				logger.Error("Failed to lookup IP address for", domain, ":", err)
				continue
			}

			// create a string list of IPs
			ipList := make([]string, len(ips))
			for i, ip := range ips {
				ipList[i] = ip.String()
			}

			// write the message to the builder
			fmt.Fprintf(&msgBuilder, "💚آی پی های دامنه : %s👇\n%s\n\n", domain, strings.Join(ipList, "\n"))
		}
