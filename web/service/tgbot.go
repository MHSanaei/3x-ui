...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		...
		case "operators":
    operatorButtons := tu.InlineKeyboard(
        tu.InlineKeyboardRow(
    ...
    t.SendAnswer(chatId, "لطفا یکی از اپراتورهای زیر را انتخاب کنید:", false, operatorButtons)
}
}
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool, buttons *telego.InlineKeyboardMarkup) {
    ...
    if isAdmin {
        params.ReplyMarkup = tu.InlineKeyboard(
            tu.InlineKeyboardRow(
