...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		...
		case "operators":
if strings.Contains(callbackQuery.Data, "operators") {
domainList := []string{"mci", "mtn", "rtl", "mkh"}
t.SendAnswer(chatId, t.getOperatorsKeyboard(domainList), true)
return
}
msg := getCommandsMessage(isAdmin)
t.SendAnswer(chatId, msg, isAdmin)
		case "mci":
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			...
			tu.InlineKeyboardButton("اپراتورها").WithCallbackData("operators"),
		),
	)
