...
func (t *Tgbot) getServerInfo(site string) (string, error) {
    // parse website url
    u, _ := url.Parse(site)

    // send a request to the website using the default http client
    resp, err := http.Get(site)
    if err != nil {
        return "", err
    }

    // extract tls state from http response
    state := resp.TLS
    ...
    // extract alpn from tls state
    alpn := state.NegotiatedProtocol

    // extract cipher suite and protocol version from tls state
    cipherSuite := state.CipherSuiteName()
    tlsVersion := tls.VersionName(state.Version)

    // extract subject and issuer information from tls state
    peerCerts := state.PeerCertificates
    subject := peerCerts[0].Subject.CommonName
    issuer := peerCerts[0].Issuer.Organization[0]

    // extract key size from tls state
    keySize := peerCerts[0].PublicKey.(*rsa.PublicKey).N.BitLen()

    // extract ip address of the server
    ip, _ := net.LookupIP(u.Hostname())
    ipStr := ip[0].String()

    // send a request to the ipinfo.io api to get information about the server location
    resp, err = http.Get(fmt.Sprintf(ipInfoUrl, ipStr, "3bd22fe89c5c42d386d84297d53389d3"))
    if err != nil {
        return "", err
    }
    ...
    // parse response and extract country and city information
    ...
    // construct message with extracted information
    msg := fmt.Sprintf("Address: %s\nALPN: %s\nIssuer: %s\nCipher Suite: %s\nTLS Version: %s\nKey Length: %d\nCountry: %s\nCity: %s\nAS Organization: %s", u.Hostname(), alpn, issuer, cipherSuite, tlsVersion, keySize, country, city, getASOrganization(ipStr))
...
func (t *Tgbot) answerCommand(message *telego.Message, chatId int64, isAdmin bool) {
    ...
    case "serverinfo":
        if len(commandArgs) == 0 {
            msg = t.I18nBot("tgbot.messages.noHostname")
        } else {
            site := commandArgs[0]
            info, err := t.getServerInfo(site)
            if err != nil {
                msg = fmt.Sprintf(t.I18nBot("tgbot.messages.serverInfoError"), err.Error())
            } else {
                msg = info
            }
        }
