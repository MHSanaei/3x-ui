...
func (bot *Tgbot) sendV2rayDownloadLink(chatID int64) error {
    url := "https://github.com/2dust/v2rayNG/releases/tag/1.8.5"
    ...
    // create a temporary file to store the downloaded V2Ray executable
    file, err := ioutil.TempFile("", "v2ray-*.zip")
    if err != nil {
        return err
    }
    ...
    // send the V2Ray executable to the user
...
func (t *Tgbot) answerCommand(message *telego.Message, chatId int64, isAdmin bool) {
	...
	case "start":
		...
		case "/download_v2ray":
    err := bot.sendV2rayDownloadLink(update.Message.Chat.ID)
    if err != nil {
        log.Println(err)
    }
	case "status":
