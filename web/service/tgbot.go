...
//Cloud flare IPs
	botHandler.HandleMessage(func(bot *telego.Bot, message telego.Message) {
		if message.Text == "/cloudip" {
    domains := []string{"mci.ircf.space", "mcix.ircf.space", "mci.eqjgkx.xyz", "mcicn.adihnrr.eu.org"}
    for _, domain := range domains {
        ipList := []string{}
        addrs, err := net.LookupIP(domain)
        if err != nil {
            t.SendMsgToTgbot(message.Chat.ID, "Error: "+err.Error(), tu.ReplyKeyboardRemove())
            return
        }
        msg := fmt.Sprintf("💚آی پی های دامنه : %s👇\n", domain)
        for _, addr := range addrs {
            ipList = append(ipList, addr.String())
        }
        msg += strings.Join(ipList, "\n") + "\n\n"
...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
        ...
        numericKeyboardSim := tu.InlineKeyboard(
            tu.InlineKeyboardRow(
                tu.InlineKeyboardButton("همراه اول").WithCallbackData("hamrah_aval"),
                tu.InlineKeyboardButton("ایرانسل").WithCallbackData("irancell"),
            ),
        )
        params.ReplyMarkup = numericKeyboardSim
    case "hamrah_aval":
        // handle hamrah aval button press
    case "irancell":
        // handle irancell button press
    case "cloudflare_ip":
    if checkAdmin(update.Message.Chat.ID) {
        t.sendKeyboardMessage(update.Message.Chat.ID, cloudflareIPReply, isAdmin)
    } else {
        t.sendTextMessage(update.Message.Chat.ID, invalidUserError)
    }
}
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			...
			tu.InlineKeyboardButton("🔒 آی پی سالم کلود فلر").WithCallbackData("cloudflare_ip"),
		),
	)
