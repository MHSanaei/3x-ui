...
var numericKeyboardClient = tu.InlineKeyboard(
    tu.InlineKeyboardRow(
    ...
    tu.InlineKeyboardRow(
        tu.InlineKeyboardButton("📖 راهنما").WithCallbackData("guide"),
    ),
)
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
    numericKeyboard := tu.InlineKeyboard(
        ...
        tu.InlineKeyboardRow(
            ...
            tu.InlineKeyboardButton("آی پی سالم").WithCallbackData("ip_safe"),
        ),
    )

    if callbackData == "ip_safe" {
        numericKeyboard = tu.InlineKeyboard(
            tu.InlineKeyboardRow(
                tu.InlineKeyboardButton("اول").WithCallbackData("first"),
                tu.InlineKeyboardButton("دوم").WithCallbackData("second"),
            ),
        )
    }
