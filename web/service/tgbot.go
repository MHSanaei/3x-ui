...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		...
		case "getips":
    // create a new inline keyboard with two buttons
    keyboard := tu.NewInlineKeyboardMarkup(
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("همراه اول").WithCallbackData(t.encodeQuery("hamrah-aval")),
            tu.InlineKeyboardButton("ایرانسل").WithCallbackData(t.encodeQuery("irancell")),
        ),
    )

    // send a message to the user with the new inline keyboard
    _, err = bot.SendMessage(msg.Chat.ID, "لطفا اپراتور خود را انتخاب کنید:", &tu.SendMessageOpt{
