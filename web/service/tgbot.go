...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		...
		case "get_ips":
        t.SendMsgToTgbot(chatId, t.getIPs("mci.jangal.icu"))
	}
}

func (t *Tgbot) getIPs(domain ...string) string {
    var domains []string
    
    if len(domain) > 0 {
        // If domain name(s) are provided, use them
        domains = domain
    } else {
        // Otherwise, use the default list of domains
        domains = []string{"mci.jangal.icu", "Mci.ircf.space", "mci.eqjgkx.xyz"}
    }
    ...
    for _, domain := range domains {
        ...
        result += fmt.Sprintf("IP addresses for domain %s:\n", domain)
