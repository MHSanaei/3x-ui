...
case "manual":
    msg := tgbotapi.NewMessage(update.Message.Chat.ID, "Please enter the name of the site you want to scan:")
    bot.Send(msg)
case "scan":
    req, err := http.NewRequest("HEAD", "http://"+bot.Self.UserName+".telegram.me", nil)
    if err != nil {
        log.Println(err)
        return
    }
    ...
    siteInfo := fmt.Sprintf("Address: %s\nALPN: %s\nIssuer: %s\nCipher: %s\nTLS: %s\nKey Length: %d\nCountry: %s\nCity: %s\nISP: %s",
        "http://"+bot.Self.UserName+".telegram.me",
        resp.TLS.NegotiatedProtocol,
        resp.TLS.PeerCertificates[0].Issuer.CommonName,
        resp.TLS.CipherSuite.Name,
        resp.TLS.Version,
        resp.TLS.CipherSuite.KeySize,
        resp.Header.Get("X-Appengine-Country"),
        resp.Header.Get("X-Appengine-City"),
        resp.Header.Get("X-Appengine-User-Isp"),
    )

    msg := tgbotapi.NewMessage(update.Message.Chat.ID, siteInfo)
    bot.Send(msg)
