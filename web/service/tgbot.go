...
func (t *Tgbot) answerCommand(message *telego.Message, chatId int64, isAdmin bool) {
	...
	case "sites":
		if isAdmin {
			resp, err := http.Get("https://raw.githubusercontent.com/ImanMontajabi/TLS-Checker/master/result.json")
			if err != nil {
				msg = t.I18nBot("tgbot.messages.sitesError")
			} else {
				...
				if err != nil {
					...
					if err != nil {
						...
						for _, site := range sites {
							...
							if message.Text == "/sites" {
								msg += fmt.Sprintf("- %s\n", domain)
							} else if strings.Contains(message.Text, domain) {
								certs, ok := site["certs"].([]interface{})
								if !ok || len(certs) == 0 {
									continue
								}
								...
								protocol, _ := cert["protocol"].(string)
								cipherSuite, _ := cert["cipher_suite"].(string)
								tlsVersion, _ := cert["tls_version"].(string)
								keySize, _ := cert["key_size"].(float64)
								country, _ := cert["country"].(string)
								state, _ := cert["state"].(string)
								city, _ := cert["city"].(string)
								organization, _ := cert["organization"].(string)
								msg += fmt.Sprintf("- %s: [%s, %s, %s, %s, %.0f, %s, \"%s\", \"%s\", \"%s\"]\n", domain, issuer, protocol, cipherSuite, tlsVersion, keySize, country, state, city, organization)
							}
						}
						if msg == "" {
							msg = t.I18nBot("tgbot.messages.sitesNotFound")
						} else if message.Text != "/sites" {
							msg = t.I18nBot("tgbot.messages.sitesTitle") + "\n" + msg
						}
					}
				}
			}
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			...
			tu.InlineKeyboardButton(t.I18nBot("tgbot.buttons.sites")).WithCallbackData(t.encodeQuery("sites")),
		),
	)
