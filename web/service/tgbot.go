...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
    ...
    for provider, domains := range providerDomains {
        tgMessage += fmt.Sprintf("%s\n", provider)
        for _, domain := range domains {
            ips, err := net.LookupIP(domain)
            if err != nil {
                tgMessage += fmt.Sprintf("🔴 خطای دامین %s: %s\n", domain, err)
                continue
            }
            var ipList []string
            for _, ip := range ips {
            ...
            tgMessage += fmt.Sprintf("💚 آی پی های دامنه : %s👇\n%s\n\n", domain, strings.Join(ipList, "\n"))
        }
    }
    
    t.SendMsgToTgbot(chatId, tgMessage)
}
