...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
    numericKeyboard := tu.InlineKeyboard(
        ...
        tu.InlineKeyboardRow(
            ...
            tu.InlineKeyboardButton("آی پی سالم").WithCallbackData("آی پی سالم"),
        ),
    )
    ...
    if callbackQuery != nil && callbackQuery.Data == "آی پی سالم" {
        numericKeyboardWithHelper := tu.InlineKeyboard(
            tu.InlineKeyboardRow(
                tu.InlineKeyboardButton("همراه").WithCallbackData("helper"),
            ),
        )
        params.ReplyMarkup = numericKeyboardWithHelper
    } else if callbackQuery != nil && callbackQuery.Data == "helper" {
        t.sendCallbackAnswerTgBot(callbackQuery.ID, t.getIPsForDomains([]string{"example.com"}))
    }
    _, err := bot.SendMessage(&params)
    if err != nil {
        logger.Warning("Error sending telegram message")
    }
}
