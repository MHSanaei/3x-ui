...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "cloudflare_ips":
    numericKeyboardCloudFlare := tu.InlineKeyboard(
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("Hamrah-e-Aval").WithCallbackData("hamrah_e_aval"),
            tu.InlineKeyboardButton("Irancell").WithCallbackData("irancell"),
        ),
    )
    params := telego.SendMessageParams{
        ChatID:    tu.ID(chatId),
        Text:      "Please select your operator:",
        ParseMode: "HTML",
        ReplyMarkup: numericKeyboardCloudFlare,
    }
    _, err := bot.SendMessage(&params)
    if err != nil {
        logger.Warning("Error sending telegram message :", err)
    }
  // Adding this curly brace will ensure that the new code is part of the same code block
  case "hamrah_e_aval":
    mciDomains := []string{"mci.ircf.space", "mcix.ircf.space", "mci.eqjgkx.xyz", "mcicn.adihnrr.eu.org"}
    var ips []string
    for _, domain := range mciDomains {
        ipAddr, err := net.LookupHost(domain)
        if err != nil {
            logger.Errorf("Error looking up IP address for %s: %v", domain, err)
            continue
        }
        ips = append(ips, fmt.Sprintf("💚آی پی های دامنه : %s👇\n%s\n", domain, strings.Join(ipAddr, "\n")))
    }
    t.SendMsgToTgbot(chatId, strings.Join(ips, "\n")))
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	...
	numericKeyboardClient := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			tu.InlineKeyboardButton("آی پی سالم کلودفلر").WithCallbackData("cloudflare_ips"),
		),
	)
	params := telego.SendMessageParams{
		ChatID:    tu.ID(chatId),
