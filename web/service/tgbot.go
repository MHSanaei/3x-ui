...
func SendAnswer(chatID int64, text string, isAdmin bool, isClient bool) {
    //...
    if isClient && strings.EqualFold(text, "ips") {
        kb := telegram.InlineKeyboardMarkup{
            InlineKeyboard: [][]telegram.InlineKeyboardButton{
                []telegram.InlineKeyboardButton{
                    {Text: "MCI", CallbackData: "mci"},
                    {Text: "MTN", CallbackData: "mtn"},
                },
            },
        }
        message := tgbotapi.NewMessage(chatID, "Please select your operator:")
