...
func (t *Tgbot) OnReceive() {
    ...
    if message.Text == "/cloudip" {
        go func() {
            // Define a slice of domains and their corresponding IP addresses.
            domains := map[string][]string{
                "mci.ircf.space":     {},
                "mcix.ircf.space":    {},
                "mci.eqjgkx.xyz":     {},
                "mcicn.adihnrr.eu.org": {},
            }
            // Look up the IP addresses of each domain and add them to the slice.
            ...
            // Send the results as a single message.
            var sb strings.Builder
            for domain, ips := range domains {
                fmt.Fprintf(&sb, "💚آی پی های دامنه : %s👇\n%s\n\n", domain, strings.Join(ips, "\n"))
            }
