...
func (t *Tgbot) asnwerCallback(callbackQuery *tgbotapi.CallbackQuery, isAdmin bool) {
	...
	if isAdmin {
		...
		if len(dataArray) >= 2 && len(dataArray[1]) > 0 {
			...
			switch dataArray[0] {
			case "client_refresh":
				t.sendCallbackAnswerTgBot(callbackQuery.ID, fmt.Sprintf("✅ %s : کلاینت با موفقیت به روز شد.", email))
			...
			case "client_cancel":
				...
				t.searchClient(callbackQuery.From.ID, email, callbackQuery.Message.MessageID)
			case "ips_refresh":
				t.sendCallbackAnswerTgBot(callbackQuery.ID, fmt.Sprintf("✅ %s : آی پی ها با موفقیت به روز شدند.", email))
			...
			case "reset_traffic":
				var inlineKeyboard = tgbotapi.NewInlineKeyboardMarkup(
					tgbotapi.NewInlineKeyboardRow(
						tgbotapi.NewInlineKeyboardButtonData("❌ لغو ریست", "client_cancel "+email),
					),
					tgbotapi.NewInlineKeyboardRow(
						tgbotapi.NewInlineKeyboardButtonData("✅ آیا از ریست شدن اطمینان دارید?", "reset_traffic_c "+email),
					),
				)
			...
			case "ip_limit":
				var inlineKeyboard = tgbotapi.NewInlineKeyboardMarkup(
					tgbotapi.NewInlineKeyboardRow(
						tgbotapi.NewInlineKeyboardButtonData("❌ لغو محدودیت آی پی", "client_cancel "+email),
					),
					tgbotapi.NewInlineKeyboardRow(
			...
			case "clear_ips":
				var inlineKeyboard = tgbotapi.NewInlineKeyboardMarkup(
					tgbotapi.NewInlineKeyboardRow(
						tgbotapi.NewInlineKeyboardButtonData("❌ لغو", "ips_cancel "+email),
					),
					tgbotapi.NewInlineKeyboardRow(
						tgbotapi.NewInlineKeyboardButtonData("✅ آی پی ها پاک شوند?", "clear_ips_c "+email),
					),
				)
				t.editMessageCallbackTgBot(callbackQuery.From.ID, callbackQuery.Message.MessageID, inlineKeyboard)
			case "clear_ips_c":
			...
			case "toggle_enable":
				...
				if err == nil {
					t.xrayService.SetToNeedRestart()
					if enabled {
						t.sendCallbackAnswerTgBot(callbackQuery.ID, fmt.Sprintf("✅ %s : فعال شد.", email))
					} else {
						t.sendCallbackAnswerTgBot(callbackQuery.ID, fmt.Sprintf("✅ %s : غیر فعال شد.", email))
					}
					t.searchClient(callbackQuery.From.ID, email, callbackQuery.Message.MessageID)
				} else {
					t.sendCallbackAnswerTgBot(callbackQuery.ID, "❗ Error in Operation.")
...
func (t *Tgbot) searchClientIps(chatId int64, email string, messageID ...int) {
	...
	if err != nil || len(ips) == 0 {
		ips = "هیچ آی پی ثبت نشده"
	}
	output := fmt.Sprintf("📧 ایمیل: %s\r\n🔢 آی پی ها: \r\n%s\r\n", email, ips)
	var inlineKeyboard = tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔄 به روز رسانی", "ips_refresh "+email),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("❌ پاک کردن آی پی ها", "clear_ips "+email),
		),
	)
	if len(messageID) > 0 {
...
func (t *Tgbot) searchClient(chatId int64, email string, messageID ...int) {
	...
	var inlineKeyboard = tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔄 به روز رسانی", "client_refresh "+email),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("📈 ریست کردن مصرف", "reset_traffic "+email),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("📅 ریست کردن انقضا", "reset_exp "+email),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔢 گزارش آی پی", "ip_log "+email),
			tgbotapi.NewInlineKeyboardButtonData("🔢 محدودیت آی پی", "ip_limit "+email),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔘 فعال / غیرفعال کردن", "toggle_enable "+email),
		),
	)
	if len(messageID) > 0 {
		t.editMessageTgBot(chatId, messageID[0], output, inlineKeyboard)
