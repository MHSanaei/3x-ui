...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := telego.InlineKeyboard{
		telego.InlineKeyboardRow(
	...
	numericKeyboardClient := telego.InlineKeyboard{
		telego.InlineKeyboardRow(
	...
	if isAdmin {
		...
		// Check if "آی پی سالم" button is pressed
		if msg == "آی پی سالم" {
			// Add Hamrah Aval and Irancell buttons
			numericKeyboardClient = append(numericKeyboardClient, telego.InlineKeyboardRow(
				tu.InlineKeyboardButton("همراه اول").WithCallbackData("mcinet"),
				tu.InlineKeyboardButton("ایرانسل").WithCallbackData("mtnirancell"),
			))
	...
	_, err := t.bot.SendMessage(&params)
	if err != nil {
