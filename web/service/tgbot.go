...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		t.SendMsgToTgbot(chatId, t.I18nBot("tgbot.commands.helpAdminCommands"))
		    case "getips":
    numericKeyboardgetips := tu.InlineKeyboard(
        tu.InlineKeyboardRow(
        ...
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("های‌وب").WithCallbackData("hwb"),
            tu.InlineKeyboardButton("آسیاتک").WithCallbackData("ast"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("شاتل").WithCallbackData("sht"),
            tu.InlineKeyboardButton("شاتل‌موبایل").WithCallbackData("shm"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("پارس‌آنلاین").WithCallbackData("prs"),
            tu.InlineKeyboardButton("مبین‌نت").WithCallbackData("mbt"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("اندیشه‌سبز").WithCallbackData("ask"),
            tu.InlineKeyboardButton("رسپینا").WithCallbackData("rsp"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("افرانت").WithCallbackData("afn"),
            tu.InlineKeyboardButton("زی‌تل").WithCallbackData("ztl"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("پیشگامان").WithCallbackData("psm"),
            tu.InlineKeyboardButton("آراکس").WithCallbackData("arx"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("سامانتل").WithCallbackData("smt"),
            tu.InlineKeyboardButton("فن‌آوا").WithCallbackData("fnv"),
        ),
        
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("دیده‌بان‌نت").WithCallbackData("dbn"),
            tu.InlineKeyboardButton("آپتل").WithCallbackData("apt"),
        ),
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("فناپ‌تلکام").WithCallbackData("fnp"),
            tu.InlineKeyboardButton("رای نت").WithCallbackData("ryn"),
            tu.InlineKeyboardButton("صبانت").WithCallbackData("sbn"),
        ),
    )
    t.SendMsgToTgbot(chatId, "اینترنت خود را انتخاب کنید:", numericKeyboardgetips)
		case "mci":
    ...
    case "mtn":
    domainList := []string{"mtn.ircf.space", "mtnx.ircf.space", "irc.eqjgkx.xyz", "irancell.fastynet.cloud", "mtn.chamburak520.online", "mtn.vcdn.online"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "mkh":
    domainList := []string{"mkh.ircf.space", "mkb.eqjgkx.xyz", "mkb.jangal.icu", "mkh.filna.cloud"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "rtl":
    domainList := []string{"rtl.ircf.space", "rtl.eqjgkx.xyz", "rtl.filna.cloud"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "hwb":
    domainList := []string{"hwb.ircf.space", "hwb.eqjgkx.xyz"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "ast":
    domainList := []string{"ast.ircf.space", "rtl.eqjgkx.xyz", "ast.eqjgkx.xyz"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "sht":
    domainList := []string{"sht.ircf.space", "sht.eqjgkx.xyz", "shtcn.adihnrr.eu.org", "shatell.cjmahdi.online", "ip.r-acc3ss.cloud", "sht.jangal.icu"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "shm":
    domainList := []string{"shm.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "prs":
    domainList := []string{"prs.ircf.space", "prs.eqjgkx.xyz"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "mbt":
    domainList := []string{"mbt.ircf.space", "mobin.nazario.top"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "ask":
    domainList := []string{"ask.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "rsp":
    domainList := []string{"rsp.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "afn":
    domainList := []string{"afn.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "ztl":
    domainList := []string{"ztl.ircf.space","ztl.eqjgkx.xyz"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "psm":
    domainList := []string{"psm.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "arx":
    domainList := []string{"arx.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "smt":
    domainList := []string{"smt.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "fnv":
    domainList := []string{"fnv.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "dbn":
    domainList := []string{"dbn.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "apt":
    domainList := []string{"apt.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
    case "fnp":
    domainList := []string{"fnp.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "ryn":
    domainList := []string{"ryn.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
   case "sbn":
    domainList := []string{"sbn.ircf.space"}
    t.SendMsgToTgbot(chatId, t.getIPsForDomains(domainList))
}
}
...
func (t *Tgbot) getIPsForDomains(domainList []string) string {
    ...
    for _, domain := range domainList {
        ...
        if err != nil {
            result += fmt.Sprintf("domain Error😔 %s: %s\n", domain, err)
            continue
        }
        result += fmt.Sprintf("%s👇\n%s\n", domain, strings.Join(getFormattedIPList(ips), "\n"))
    }
    return result
}

func getFormattedIPList(ips []net.IP) []string {
    var formattedIPs []string
    for _, ip := range ips {
        formattedIPs = append(formattedIPs, ip.String())
    }
    return formattedIPs
}
...
// Function to get list of URLs to perform automatic TLS checks on
func getUrls() []string {
    url := "https://raw.githubusercontent.com/Ptechgithub/TLS-Checker/master/file2.csv"
    response, err := http.Get(url)
    if err != nil {
        log.Fatal("Error fetching URL list:", err)
    }
...
func handleScanCommand(update tgbotapi.Update) {
    msg := update.Message
    chatID := msg.Chat.ID

    // Create the inline keyboard for the scan options
    keyboard := tgbotapi.NewInlineKeyboardMarkup(
        tgbotapi.NewInlineKeyboardRow(
            tgbotapi.NewInlineKeyboardButtonData("Manual", "manual"),
            tgbotapi.NewInlineKeyboardButtonData("Automatic", "auto"),
        ),
    )

    // Send message with the inline keyboard to the user
    message := tgbotapi.NewMessage(chatID, "Please select a scan type:")
    message.ReplyMarkup = keyboard
    bot.Send(message)
}

// Handle the callback query from the inline keyboard for manual scans
func handleManualScanQuery(update tgbotapi.Update) {
    query := update.CallbackQuery
    chatID := query.Message.Chat.ID

    // Get the URL to scan from the user
    message := tgbotapi.NewMessage(chatID, "Please enter a URL to scan:")
    urlResponse, err := bot.Request(message)
    if err != nil {
        log.Printf("Error getting URL input from user: %v", err)
        return
    }
    url := urlResponse.Text

    // Perform the TLS check on the URL
    result := tlsCheck(url)

    // Format the results into a message
    messageText := fmt.Sprintf("TLS check results for %s:\n"+
        "Country: %s\nCity: %s\nISP: %s",
        url, result.Country, result.City, result.ISP)

    // Send the message back to the user
    response := tgbotapi.NewMessage(chatID, messageText)
    bot.Send(response)
}

// Handle the callback query from the inline keyboard for automatic scans
func handleAutoScanQuery(update tgbotapi.Update) {
    query := update.CallbackQuery
    chatID := query.Message.Chat.ID
    ...
    // Perform the automatic TLS scan on all URLs
    ...
    // Format the results into a message
    messageText := "Automatic TLS scan results:\n"
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			tu.InlineKeyboardButton(t.I18nBot("tgbot.buttons.getips")).WithCallbackData(t.encodeQuery("getips")),
			tu.InlineKeyboardButton(t.I18nBot("tgbot.buttons.scan")).WithCallbackData(t.encodeQuery("scan")),
		),
	)
