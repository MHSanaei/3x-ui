...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := telego.InlineKeyboardMarkup{
		InlineKeyboard: [][]telego.InlineKeyboardButton{
			...
			},
			{
				...
				tu.InlineKeyboardButton("آی پی سالم").WithCallbackData("cloudips"),
			},
		},
	}
	numericKeyboardClient := telego.InlineKeyboardMarkup{
		InlineKeyboard: [][]telego.InlineKeyboardButton{
			{
	...
	var additionalButtons []telego.InlineKeyboardButton
	switch msg {
	case "آی پی سالم":
		additionalButtons = []telego.InlineKeyboardButton{
			tu.InlineKeyboardButton("irancell").WithCallbackData("ip_irancell"),
			tu.InlineKeyboardButton("mci").WithCallbackData("ip_mci"),
		}
	default:
		additionalButtons = []telego.InlineKeyboardButton{}
	}
	...
	if isAdmin {
		...
		params.InlineKeyboard = append(numericKeyboard.InlineKeyboard, additionalButtons)
	} else {
		params.ReplyMarkup = numericKeyboardClient
		params.InlineKeyboard = append(numericKeyboardClient.InlineKeyboard, additionalButtons)
	}
