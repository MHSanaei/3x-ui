...
case "get_ips":
    ...
    irancellList := []string{"mtn.if.space", "mci.ircf.space"}

    mciResult := t.getIPsForDomains(mciList)
    irancellResult := t.getIPsForDomains(irancellList)

    result := "⭕️ همراه اول ⭕️\n"
    result += mciResult + "\n\n"

    result += "⭕️ ایرانسل ⭕️\n"
    result += irancellResult
