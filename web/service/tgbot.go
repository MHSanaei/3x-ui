...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		...
		case "get_ips":
    mciList := []string{"mci.ircf.space", "mcix.ircf.space", "mci.eqjgkx.xyz", "mcicn.adihnrr.eu.org", "mci.vcdn.online", "mci.ma7ta.online", "mci.boby66600.eu.org", "mci.jangal.icu", "mci.filna.cloud"}
    irancellList := []string{"mtn.if.space", "mci.ircf.space", "hsh.xjsu.space"}

    var domains []string
    domains = append(domains, mciList...)
    domains = append(domains, irancellList...)

    sort.Strings(domains)

    var result, title string
    title = "⭕️ همراه اول ⭕️ 👇\n\n"
    for _, domain := range domains {
        ips, err := net.LookupIP(domain)
        if err != nil {
            result += fmt.Sprintf("خطای دامنه %s: %s\n", domain, err)
            continue
        }
        var ipList []string
        for _, ip := range ips {
        ...
        if strings.Contains(domain, "mci") {
            result += "⭕️ همراه اول ⭕️\n"
        } else {
            result += "⭕️ ایرانسل ⭕️\n"
        }
    ...
    t.SendMsgToTgbot(chatId, title+result)
}
