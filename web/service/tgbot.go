...
func (t *Tgbot) OnReceive() {
	...
	// Handle new button named "آی پی سالم"
	botHandler.HandleMessage(func(bot *telego.Bot, message telego.Message) {
		if message.Text == "آی پی سالم" {
			// Create a custom keyboard with two buttons
			buttons := [][]telego.KeyboardButton{
				{
					telego.KeyboardButton{Text: "همراه"},
					telego.KeyboardButton{Text: "ایرانسل"},
				},
			}
			inlineKeyboard := telego.ReplyMarkup{
				InlineKeyboard: telego.NewMarkupWithButtons(buttons),
			}

			// Send the new keyboard to the user
			t.SendMsgToTgbot(message.Chat.ID, "لطفا یکی از دکمه‌های زیر را انتخاب کنید", inlineKeyboard)
		}
	}, th.TextEqual("آی پی سالم"))

	// Handle clicks on the new buttons
	botHandler.HandleCallbackQuery(func(bot *telego.Bot, query telego.CallbackQuery) {
		switch query.Data {
		case "hamrah":
			// Handle click on the "hamrah" button
		case "irancell":
			// Handle click on the "irancell" button
		default:
			logger.Warning("Unknown callback data:", query.Data)
		}
	}, th.CallbackData())
