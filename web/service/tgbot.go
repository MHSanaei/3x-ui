...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
    ...
    numericKeyboardgetips := tu.InlineKeyboard(
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("دستی").WithCallbackData("manualtls"),
            ),
        )
        
        func handleManualTLS(bot *tgbotapi.BotAPI, update tgbotapi.Update) {
    // Send a message to the user asking for the site name
    msg := tgbotapi.NewMessage(update.Message.Chat.ID, "Please enter the address of the website:")
    bot.Send(msg)

    // Wait for the user's response
    siteNameMsg := waitForReply(bot, update.Message.Chat.ID)

    // Get the details for the site using the tlsinfo library (you will need to install it first)
    details, err := tlsinfo.GetTLSInfo(siteNameMsg.Text)
    if err != nil {
        log.Println("Error getting TLS info:", err)
        return
    }

    // Generate a reply message with the site details
...
// Helper function to wait for a reply from the user
func waitForReply(bot *tgbotapi.BotAPI, chatID int64) tgbotapi.Message {
    ...
    return tgbotapi.Message{} // This should never happen
}
