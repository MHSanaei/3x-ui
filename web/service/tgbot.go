...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "get_ips":
            t.getIPsForDomains(chatId)
        case "get_mci_ips":
            t.getIPsForMCI(chatId) // call the getIPsForMCI function here
        }
        // AnswerCallbackQuery to close the loading state of button in Telegram app
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
    numericKeyboard := tu.InlineKeyboard(
        ...
        tu.InlineKeyboardRow(
            ...
            tu.InlineKeyboardButton("آی پی سالم").WithCallbackData("get_ips").WithInlineKeyboard(
                tu.InlineKeyboardRow(
                    tu.InlineKeyboardButton("mci").WithCallbackData("get_mci_ips"),
                ),
            ),
        ),
    )
