...
func (t *Tgbot) asnwerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "commands":
		...
		case "get_ips":
    var mciDomainList = []string{"mci.ircf.space", "mcix.ircf.space", "mci.eqjgkx.xyz", "mcicn.adihnrr.eu.org", "mci.vcdn.online", "mci.ma7ta.online", "mci.boby66600.eu.org", "mci.jangal.icu", "mci.filna.cloud"}
    var mtnDomainList = []string{"mtn.ircf.space", "nsjud.jsus.ir"}
    var result strings.Builder

    if len(mciDomainList) > 0 {
        result.WriteString("⭕️ همراه اول ⭕️ 👇\n\n")
        result.WriteString(t.getIPsForDomains(mciDomainList))
    }

    if len(mtnDomainList) > 0 {
        result.WriteString("\n\n⭕️ ایرانسل ⭕️ 👇\n\n")
        result.WriteString(t.getIPsForDomains(mtnDomainList))
    }
...
func (t *Tgbot) getIPsForDomains(domainList []string) string {
    ...
    for _, domain := range domainList {
        ...
        if err != nil {
            result.WriteString(fmt.Sprintf("خطای دامین %s: %s\n", domain, err))
            continue
        }
        var ipList []string
        for _, ip := range ips {
        ...
        result.WriteString(fmt.Sprintf("💚 آی پی های دامنه  %s:\n%s\n\n", domain, strings.Join(ipList, "\n")))
    }
