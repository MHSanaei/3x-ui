		...
		if err != nil {
			...
			if err != nil {
				...
				if err != nil {
					...
					randomSite := sites[keys[rand.Intn(len(keys))]]
					domain := keys[rand.Intn(len(keys))]
					cert := randomSite[0].(map[string]interface{})
					...
					msg = fmt.Sprintf("%s:\n- Issuer: %s\n- Protocol: %s\n- Cipher Suite: %s\n- TLS Version: %s\n- Key Size: %.0f\n- Country: %s\n- State: %s\n- City: %s\n- Organization: %s", domain, issuer, protocol, cipherSuite, tlsVersion, keySize, country, state, city, organization)
				}
			}
		}

		// send the response to the user using SendMsgToTgbot
