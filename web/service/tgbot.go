...
case "scan":
    numericKeyboardgetips := tu.InlineKeyboard(
        tu.InlineKeyboardRow(
            tu.InlineKeyboardButton("اتوماتیک").WithCallbackData("auto"),
            tu.InlineKeyboardButton("دستی").WithCallbackData("manual"),
        ),
    )
    if callbackData == "manual" {
        _, err := b.Send(msg.Sender, "لطفا نام وب سایت را وارد کنید:")
        if err != nil {
            log.Printf("Error sending message to chat - %v", err)
        }
        // Set a state variable to remember that we're waiting for the website name
        b.SetUserState(msg.Sender, "waiting_for_website_name")
    } else if callbackData == "auto" {
        // Perform automatic scanning
        // ...
    }

// Handle messages sent in response to the "waiting_for_website_name" state
case tgbotapi.UpdateTypeMessage:
    if userState == "waiting_for_website_name" {
        websiteName := update.Message.Text
        result, err := scanWebsite(websiteName)
        if err != nil {
            log.Printf("Error scanning website - %v", err)
            return
        }
        _, err = b.Send(msg.Sender, result)
        if err != nil {
            log.Printf("Error sending message to chat - %v", err)
        }
        // Clear the state variable now that we're done with it
        b.ClearUserState(msg.Sender)
    }
}

// Function to scan a website and extract the desired information
func scanWebsite(websiteName string) (string, error) {
    // ...
    // Use a library like GoQuery or net/http to fetch the website HTML
    // Parse the HTML to extract the desired information (e.g. using regular expressions)
    // ...
    result := fmt.Sprintf("Address: %s\nALPN: %s\nIssuer: %s\nCipher: %s\nTLS: %s\nKey length: %d\nCountry: %s\nISO code: %s\nCity: %s\nISP: %s", address, alpn, issuer, cipher, TLS, key_length, country, iso_code, city, isp)
    return result, nil
}
