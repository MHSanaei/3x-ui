...
func handleIspCommand(chatID int64) {
    replyMarkup := tgbotapi.NewReplyKeyboard(
        tgbotapi.NewKeyboardButtonRow(
            tgbotapi.NewKeyboardButton("همراه اول"),
            tgbotapi.NewKeyboardButton("ایرانسل"),
        ),
        tgbotapi.NewKeyboardButtonRow(
            tgbotapi.NewKeyboardButton("رایتل"),
            tgbotapi.NewKeyboardButton("مخابرات"),
        ),
    )
    replyMarkup.ResizeKeyboard = true
    
    message := tgbotapi.NewMessage(chatID, "🔠 لطفا اپراتور خود را انتخاب کنید ، لیست ای پی ها در لحظه به روز میشوند•")
    message.ReplyMarkup = replyMarkup

    bot.Send(message)
}

func handleOperatorSelection(chatID int64, operator string) {
    var domains []string
    switch operator {
    case "همراه اول":
        domains = []string{
    ...
    for _, domain := range domains {
        ...
        if err != nil { // Handle DNS resolution error
            message += fmt.Sprintf("😔این دامین هیچ IP ثبت شده ای ندارد. %s.\n\n", domain)
            continue
        }
        message += fmt.Sprintf("💚آی پی های دامنه : %s👇\n%s\n\n", domain, strings.Join(ips.String(), "\n"))
    }
...
func handleMessage(update tgbotapi.Update) {
    ...
    switch messageText {
    case "آی پی سالم":
        handleIspCommand(chatID)
    case "همراه اول", "ایرانسل", "رایتل", "مخابرات":
        handleOperatorSelection(chatID, messageText)
    default:
        // Do nothing
    }
}
