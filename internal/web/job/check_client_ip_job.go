...
// getInboundsByEmail resolves every inbound a client email is attached to. Like
// getInboundByEmail it prefers the exact clients/client_inbounds relation and
// falls back to a settings JSON scan, but returns all matches: the IP-limit
// disconnect must drop the over-limit client on each of its inbounds, since Xray
// meters every (client, inbound) pair under its own per-attachment identity.
...
// its inbounds, but Xray meters each (client, inbound) pair separately, so a
// single-inbound remove/re-add would leave the over-limit connection alive on the
// client's other inbounds. primary is the inbound the caller already resolved; its
// parsed clients are reused to avoid a reparse. A settings match that isn't a real
// attachment is harmless: disconnectClientTemporarily exact-matches the email and
