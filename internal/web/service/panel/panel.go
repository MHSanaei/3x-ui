...
// this fall back to the panel's own configured Panel Outbound bridge, if
// any. Returns nil (no override) when neither applies, so both call sites
// stay no-ops on an ordinary direct-connection install.
//
...
// lose it for this one hop even without Panel Outbound in the picture.
