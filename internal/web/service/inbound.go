...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	db := database.GetDB()
	portKey := portLockKey{nodeScope: inboundNodeScope(inbound), port: inbound.Port}
	unlockPort := lockPortReservationKeys(portKey)
	defer unlockPort()
	needRestart := false
	var postCommitApply func()
	err = db.Transaction(func(tx *gorm.DB) error {
		markDirty := false
		if err := lockPortReservationKeysTx(tx, portKey); err != nil {
			return err
		}
...
func (s *InboundService) DelInbound(id int) (bool, error) {
	...
	if err := db.Transaction(func(tx *gorm.DB) error {
		...
		if err := deleteInboundPortReservationsTx(tx, id); err != nil {
			return err
		}
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	oldPortKey := portLockKey{nodeScope: inboundNodeScope(oldInbound), port: oldInbound.Port}
	newPortKey := portLockKey{nodeScope: inboundNodeScope(inbound), port: inbound.Port}
	unlockPort := lockPortReservationKeys(oldPortKey, newPortKey)
	defer unlockPort()
	...
	txErr := runSerializedTx(func(tx *gorm.DB) error {
		if err := lockPortReservationKeysTx(tx, oldPortKey, newPortKey); err != nil {
			return err
		}
		...
		if err := replaceInboundPortReservationsTx(tx, oldInbound); err != nil {
			return err
		}
