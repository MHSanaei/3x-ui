...
type CustomGeoService struct {
	...
	getPanelProxy    func() string
	// Reschedule is called when the cron schedule is saved via the API so the
	// panel can apply the new schedule without a restart.
	Reschedule func(newCron string)
}
...
func NewCustomGeoService() *CustomGeoService {
	...
	s.getPanelProxy = ss.PanelEgressProxyURL
	return s
}
...
func (s *CustomGeoService) httpClient(timeout time.Duration) *http.Client {
	...
	if s.getPanelProxy != nil {
		proxyURL = strings.TrimSpace(s.getPanelProxy())
	}
	if proxyURL != "" {
...
// GetSchedule returns the current cron schedule for auto-update.
func (s *CustomGeoService) GetSchedule() (string, error) {
	return (&service.SettingService{}).GetCustomGeoCron()
}

// SetSchedule saves the cron schedule and applies it immediately via Reschedule callback.
func (s *CustomGeoService) SetSchedule(cronExpr string) error {
	if err := (&service.SettingService{}).SetCustomGeoCron(cronExpr); err != nil {
		return err
	}
	if s.Reschedule != nil {
		s.Reschedule(cronExpr)
	}
	return nil
}
