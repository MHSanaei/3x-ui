...
func (s *InboundService) normalizeAmneziaWGSettings(inbound *model.Inbound) error {
	logger.Debugf("[awg-debug] normalizeAmneziaWGSettings called for inbound ID=%d, settings empty=%v", inbound.Id, inbound.Settings == "" || inbound.Settings == "null" || inbound.Settings == "{}")
	...
	if inbound.Settings == "" || inbound.Settings == "null" || inbound.Settings == "{}" {
		...
		logger.Debugf("[awg-debug] generated new settings, pubKey=%s", params.PublicKey)
		return nil
	}

	var parsed amneziawg.SettingsInbound
	if err := json.Unmarshal([]byte(inbound.Settings), &parsed); err != nil {
		logger.Debugf("[awg-debug] failed to parse AWG settings: %v", err)
		return fmt.Errorf("failed to parse AWG settings: %w", err)
	}
	if parsed.Server == nil {
		logger.Debugf("[awg-debug] server config missing from settings, generating new")
		params := amneziawg.GenerateAWGParams()
		params.ServerPort = inbound.Port
		fillAWGKeys(&params)
		...
		inbound.Settings = string(bs)
		logger.Debugf("[awg-debug] regenerated settings with server, pubKey=%s", params.PublicKey)
	} else {
		logger.Debugf("[awg-debug] server already present, pubKey=%s privateKey_len=%d",
		...
		if len(parsed.Server.PrivateKey) != 44 {
			logger.Debugf("[awg-debug] privateKey len=%d (expected 44), regenerating keys", len(parsed.Server.PrivateKey))
...
func fillAWGKeys(s *amneziawg.ServerConfig) {
	if s.PublicKey != "" && strings.HasSuffix(s.PublicKey, "=") {
		logger.Debugf("[awg-debug] fillAWGKeys: valid padded publicKey already set, skipping")
		return
	}
	logger.Debugf("[awg-debug] fillAWGKeys: generating new WireGuard key pair")
	if priv, pub, psk, err := amneziawg.GenerateWireGuardKeyPair(); err == nil {
		s.PrivateKey = priv
		s.PublicKey = pub
		s.PSK = psk
		logger.Debugf("[awg-debug] fillAWGKeys: generated keys ok, pubKey=%s", pub)
	} else {
		logger.Debugf("[awg-debug] fillAWGKeys: GenerateWireGuardKeyPair failed: %v", err)
	}
}
