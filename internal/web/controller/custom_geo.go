...
func (a *CustomGeoController) getSchedule(c *gin.Context) {
	cron, err := a.customGeoService.GetSchedule()
	if err != nil {
		jsonMsg(c, "get schedule", err)
		return
	}
	jsonObj(c, cron, nil)
}

type customGeoScheduleForm struct {
	Cron string `json:"cron" form:"cron"`
}

func (a *CustomGeoController) setSchedule(c *gin.Context) {
	var form customGeoScheduleForm
	if err := c.ShouldBind(&form); err != nil {
		jsonMsg(c, "set schedule", err)
		return
	}
	err := a.customGeoService.SetSchedule(form.Cron)
	jsonMsg(c, I18nWeb(c, "pages.index.customGeoCronSaved"), err)
}
