...
func allocatePort(db *gorm.DB) (int, error) {
	...
	for port := 30000; port <= 39999; port++ {
		...
		ln, err := listenConfig.Listen(context.Background(), "tcp", fmt.Sprintf("127.0.0.1:%d", port))
		if err != nil {
			continue
		}
		_ = ln.Close()
...
func runConcurrent(tags []string, fn func(string) error) error {
	...
	for index, tag := range tags {
		...
		go func() {
			...
			if err := fn(tag); err != nil {
				errs[index] = fmt.Errorf("%s: %w", tag, err)
			}
		}()
	}
	wg.Wait()
...
func (m *Manager) RestartAll() error {
	...
	return runConcurrent(tags, m.Restart)
}
...
func (m *Manager) StopAll() error {
	...
	m.mu.Unlock()
	return runConcurrent(tags, m.Stop)
}
