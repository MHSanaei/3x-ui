...
func releaseTagPath() string {
	return filepath.Join(config.GetBinFolderPath(), "naive.version")
}

func storeInstalledReleaseTag(version string) error {
	if err := ValidateVersion(version); err != nil {
		return err
	}
	return os.WriteFile(releaseTagPath(), []byte(version+"\n"), 0o644)
}
...
func InstalledReleaseTag() string {
	...
	body, err := os.ReadFile(releaseTagPath())
	if err != nil {
		return ""
	}
	tag := strings.TrimSpace(string(body))
	if ValidateVersion(tag) != nil {
		return ""
	}
	return tag
}
...
func LogPath(tag string) string {
	return filepath.Join(config.GetLogFolder(), "naive-"+tag+".log")
}

func RemoveLog(tag string) error {
	if err := ValidateTag(tag); err != nil {
		return err
	}
	if err := os.Remove(LogPath(tag)); err != nil && !errors.Is(err, os.ErrNotExist) {
		return err
	}
	return nil
}
...
func RemoveAllLogs() error {
	matches, err := filepath.Glob(filepath.Join(config.GetLogFolder(), "naive-*.log"))
	if err != nil {
		return err
	}
	var errs []error
...
func killStaleByTag(tag string) {
	...
	cfg := ConfigPath(tag)
	_ = exec.CommandContext(context.Background(), "pkill", "-f", cfg).Run()
}
...
func Start(tag string, cfg Config) (*Process, error) {
	...
	killStaleByTag(tag)
	cmd := exec.CommandContext(context.Background(), BinaryPath(), path)
...
func UninstallBinary() error {
	...
	staged, err := filepath.Glob(filepath.Join(config.GetBinFolderPath(), ".naive-*"))
	if err != nil {
		errs = append(errs, err)
	} else {
	...
	if err := os.Remove(releaseTagPath()); err != nil && !errors.Is(err, os.ErrNotExist) {
		errs = append(errs, err)
	}
	return errors.Join(errs...)
}
