...
func TestInitDB_GeneratesRandomSubscriptionPort(t *testing.T) {
	loadSubPort := func(dbPath string) int {
		t.Helper()
		if err := InitDB(dbPath); err != nil {
			t.Fatalf("InitDB failed: %v", err)
		}
		...
		var setting model.Setting
		if err := db.Where("key = ?", "subPort").First(&setting).Error; err != nil {
			t.Fatalf("read subPort: %v", err)
		}
		var port int
		for _, r := range setting.Value {
			if r < '0' || r > '9' {
				t.Fatalf("subPort %q is not a number", setting.Value)
			}
	...
	firstPort := loadSubPort(dbPath)
	if firstPort < 1024 || firstPort > 62000 {
		t.Fatalf("subPort %d out of expected range [1024, 62000]", firstPort)
	}

	// Preserved across restart
	reloadedPort := loadSubPort(dbPath)
	if reloadedPort != firstPort {
		t.Fatalf("subPort changed after restart: %d vs %d", reloadedPort, firstPort)
	}
}

func TestInitDB_PreservesExistingSubscriptionPort(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "x-ui.db")
	if err := InitDB(dbPath); err != nil {
		t.Fatalf("InitDB failed: %v", err)
	}

	// Explicitly set subPort to 2096 (or custom 9999) as an existing installation would have
	if err := db.Model(&model.Setting{}).Where("key = ?", "subPort").Update("value", "9999").Error; err != nil {
		t.Fatalf("update subPort: %v", err)
	}
	if err := CloseDB(); err != nil {
		t.Fatalf("CloseDB: %v", err)
	}

	// Reopen DB — subPort must remain 9999
	if err := InitDB(dbPath); err != nil {
		t.Fatalf("InitDB reopened failed: %v", err)
	}
	...
	if setting.Value != "9999" {
		t.Fatalf("subPort overwritten: got %q, want %q", setting.Value, "9999")
	}
}
