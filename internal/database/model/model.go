...
// next full restart. Idempotent: a no-op once "peers" exists or "clients" is
// absent, so the full-config path that already populated peers is unaffected.
