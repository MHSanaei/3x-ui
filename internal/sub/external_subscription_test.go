...
func TestFetchSubscriptionLinksSharesConcurrentRefresh(t *testing.T) {
	...
	for range callers {
		wg.Go(func() {
			results <- fetchSubscriptionLinks(0, srv.URL)
		})
	}
...
func TestFetchSubscriptionLinksBoundsCacheSize(t *testing.T) {
	...
	for i := range subscriptionCacheCapacity + 1 {
		links := fetchSubscriptionLinks(0, srv.URL+"?id="+strconv.Itoa(i))
		if len(links) != 1 {
...
func TestFetchSubscriptionLinksSharesStaleResultAfterRefreshFailure(t *testing.T) {
	...
	for range callers {
		wg.Go(func() {
			results <- fetchSubscriptionLinks(0, staleURL)
		})
	}

	time.Sleep(100 * time.Millisecond)
	if links := fetchSubscriptionLinks(0, srv.URL+"/fresh"); len(links) != 1 || links[0] != "vless://fresh@example.com:443" {
