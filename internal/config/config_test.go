...
func TestGetSubPortOverride(t *testing.T) {
	tests := []struct {
		name       string
		value      string
		set        bool
		...
		{name: "unset", set: false, wantPort: 0, configured: false},
		{name: "empty", value: "", set: true, wantPort: 0, configured: false},
		{name: "whitespace only", value: "   ", set: true, wantPort: 0, configured: false},
		{name: "minimum", value: "1", set: true, wantPort: 1, configured: true},
		{name: "default sub port", value: "2096", set: true, wantPort: 2096, configured: true},
		{name: "surrounding whitespace", value: " 8443 ", set: true, wantPort: 8443, configured: true},
		{name: "maximum", value: "65535", set: true, wantPort: 65535, configured: true},
		{name: "zero", value: "0", set: true, configured: true, wantErr: true},
		{name: "above maximum", value: "65536", set: true, configured: true, wantErr: true},
		{name: "negative", value: "-1", set: true, configured: true, wantErr: true},
		{name: "non-numeric", value: "abc", set: true, configured: true, wantErr: true},
		{name: "decimal", value: "8443.0", set: true, configured: true, wantErr: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.set {
				t.Setenv("XUI_SUB_PORT", tt.value)
			} else {
				original, existed := os.LookupEnv("XUI_SUB_PORT")
				if err := os.Unsetenv("XUI_SUB_PORT"); err != nil {
					t.Fatalf("unset XUI_SUB_PORT: %v", err)
				}
				t.Cleanup(func() {
					if existed {
						_ = os.Setenv("XUI_SUB_PORT", original)
					} else {
						_ = os.Unsetenv("XUI_SUB_PORT")
					}
				})
			}

			port, configured, err := GetSubPortOverride()
			if port != tt.wantPort {
				t.Errorf("port = %d, want %d", port, tt.wantPort)
			}
			if configured != tt.configured {
